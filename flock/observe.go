...
// out of the acquire context. flock has no on-disk marker body,
// so the TraceID is logged via the slog hook only — useful when
// correlating Acquire/Release log lines with the originating
// request.
...
func (o observeOptions) recordAcquire(name, outcome string, duration time.Duration, traceID string, err error) {
	...
	if o.logger != nil {
		...
		o.logger.LogAttrs(context.Background(), slog.LevelInfo, "flock acquire", attrs...)
	}
}
...
func (o observeOptions) recordHold(name string, duration time.Duration) {
	...
	if o.logger != nil {
		o.logger.LogAttrs(context.Background(), slog.LevelInfo, "flock release",
			slog.String("name", name),
