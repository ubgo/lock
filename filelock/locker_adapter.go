...
// service constructor and the same code works against filelock,
// flock, redislock, etcdlock, or memlock.
//
// If you only ever use filelock, you don't need this. Call
// [Factory.Acquire] / [Factory.WithLock] directly.
//
// Per-call options cannot be passed through the [lock.Locker]
// interface (it has no opts argument by design — see RFC in the
// ubgo/locker module). To use per-call options, hold on to the
// concrete *Factory and call its methods.
...
// Acquire ignores the supplied name (it always uses l.Name()). This is
...
func (a factoryAdapter) Acquire(ctx context.Context, name string) (lock.Holder, error) {
	...
	if err != nil {
		// Translate the package-specific sentinel into the shared one
		// so callers checking errors.Is(err, lock.ErrLocked) work.
