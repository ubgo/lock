...
// out of the acquire context. The returned ID, when non-empty, is
// stored as the etcd lock key's value (alongside the session ID)
// so operators inspecting `etcdctl get <key>` see the trace that
// originated the lock.
...
func (o observeOptions) recordAcquire(name, outcome string, duration time.Duration, traceID string, err error) {
	...
	if o.logger != nil {
		...
		o.logger.LogAttrs(context.Background(), slog.LevelInfo, "etcdlock acquire", attrs...)
	}
}
...
func (o observeOptions) recordHold(name string, duration time.Duration) {
	...
	if o.logger != nil {
		o.logger.LogAttrs(context.Background(), slog.LevelInfo, "etcdlock release",
			slog.String("name", name),
